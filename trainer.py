...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    self.optimizer.step()
                    if self.state.global_step <= 30 or self.state.global_step % 30 == 0:
                        if isinstance(self.model, PeftModel):
                            save_path = os.path.join(self.args.output_dir, f"postupdate_adapter_step{self.state.global_step}")
                    ...
                    loss_cur = self._maybe_log_save_evaluate(tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval)
                    
                    if self.is_local_process_five():
                        wandb.log({"step": self.state.global_step, "train_loss": loss_cur})
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            loss_cur = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["loss"] = loss_cur
    ...
    def is_local_process_five(self) -> bool:
        """
        Whether or not this process is the local (e.g., on one machine if training in a distributed fashion on several
        machines) main process.
        """
        return int(os.environ.get("LOCAL_RANK", 0)) == 1
