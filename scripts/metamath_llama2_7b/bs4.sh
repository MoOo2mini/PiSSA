        ...
        #     --merge True \
            ...
            do
                ...
                if [ -d "$CKPT_PATH" ]; then
                    ...
                    CUDA_VISIBLE_DEVICES=4,5,6,7 python utils/merge_adapter.py \
                        --base_model $RES_MODEL \
                        --adapter $CKPT_PATH \
                        --output_path $OUTPUT_PATH_MER

                    CUDA_VISIBLE_DEVICES=4,5,6,7 python utils/gen_vllm.py \
                        --model $OUTPUT_PATH_MER \
                        --sub_task metamath \
                        --output_file $OUTPUT_PATH_MER/metamath_response.jsonl

                    CUDA_VISIBLE_DEVICES=4,5,6,7 python utils/test_acc.py \
            ...
            for i in $(seq 2700 1920 $MAX_STEP)
            do
                CKPT_NAME="postupdate_adapter_step$i"
                CKPT_PATH="${OUTPUT_PATH}/${CKPT_NAME}"
                OUTPUT_PATH_MER="${OUTPUT_PATH}_ckpt$i"
